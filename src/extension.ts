...
export async function activate(context: vscode.ExtensionContext) {
	outputChannel = vscode.window.createOutputChannel(agentName)
	...
	try {
		if (!previousVersion || currentVersion !== previousVersion) {
			Logger.log(`${agentName} version changed: ${previousVersion} -> ${currentVersion}. First run or update detected.`)
			const lastShownPopupNotificationVersion = context.globalState.get<string>("agentLastPopupNotificationVersion")
			...
			await context.globalState.update("agentVersion", currentVersion)
		}
	} catch (error) {
	...
	const openClineInNewTab = async () => {
		...
		const panel = vscode.window.createWebviewPanel(WebviewProvider.tabPanelId, agentName, targetCol, {
			enableScripts: true,
	...
	*/
	...
	context.subscriptions.push(
		vscode.commands.registerCommand(addToChatCommand, async (range?: vscode.Range, diagnostics?: vscode.Diagnostic[]) => {
			await vscode.commands.executeCommand(focusChatInputCommand) // Ensure Cline is visible and input focused
		...
		vscode.commands.registerCommand(focusChatInputCommand, async () => {
			...
			if (activeWebviewProvider?.getWebview() && activeWebviewProvider.getWebview().hasOwnProperty("reveal")) {
				...
				if (!activeWebviewProvider) {
					...
					await vscode.commands.executeCommand(openNewTabCommand)
