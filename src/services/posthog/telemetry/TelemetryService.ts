				...
				if (isVsCodeHost) {
					void HostProvider.window
						.showMessage({
							...
							message: `Anonymous ${agentName} error and usage reporting is enabled, but VSCode telemetry is disabled. To enable error and usage reporting for this extension, enable VSCode telemetry in settings.`,
							options: {
								items: ["Open Settings"],
							},
						})
					...
					void HostProvider.window.showMessage({
						type: ShowMessageType.WARNING,
						message: `Anonymous ${agentName} error and usage reporting is enabled, but host telemetry is disabled.`,
					})
				}
			}
		}
