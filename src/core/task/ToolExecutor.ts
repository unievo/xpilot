...
import { agentName, extensionId } from "@/shared/Configuration"
			...
			case "get_mcp_tool_input_schema": {
				...
				try {
					if (block.partial) {
						...
						const connectedServers = this.mcpHub.getServers().filter((server) => server.status === "connected")
						...
						this.pushToolResult(formatResponse.toolResult(schemaResult), block)
						//await this.saveCheckpoint()
						break
					}
				} catch (error) {
					await this.handleError("getting MCP tool input schema", error, block)
					//await this.saveCheckpoint()
					break
				}
			}
