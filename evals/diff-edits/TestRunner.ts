...
import { EvalOpenRouterModelInfo, loadOpenRouterModelData } from "./openRouterModelsHelper" // Added import
import { AssistantMessageContent, parseAssistantMessageV2 } from "./parsing/parse-assistant-message-06-06-25"
...
import { ConstructSystemPromptFn, InputMessage, ProcessedTestCase, SystemPromptDetails, TestCase, TestConfig } from "./types"
...
class NodeTestRunner {
	...
	constructor(isReplay: boolean, provider: string = "openrouter") {
		...
		if (!isReplay) {
			if (provider === "openai") {
				...
				if (!this.apiKey) {
					throw new Error(
						"OPENROUTER_API_KEY environment variable not set for a non-replay run with OpenRouter provider.",
					)
				}
			}
		}
	 ...
	 */
	async storeReplayResultInDatabase(
		...
		try {
			...
			const resultInput: CreateResultInput = {
				...
				parsed_tool_call_json: replayResult.toolCalls
					? JSON.stringify(replayResult.toolCalls)
					: originalResult.parsed_tool_call_json,
			}
	 ...
	 */
	public estimateTokens(messages: Anthropic.Messages.MessageParam[]): number {
		// Made public
		let totalText = ""
		...
		for (const originalResult of originalResults) {
			...
			if (isValidOriginalAttempt) {
				...
				if (!originalCase) {
					log(
						isVerbose,
						`  [WARN] Replay for result ${originalResult.result_id}: Could not find original case. Copying original result.`,
					)
					...
					const parsedToolCall = originalResult.parsed_tool_call_json
						? JSON.parse(originalResult.parsed_tool_call_json)[0]
						: null
				...
				log(
					isVerbose,
					`  [SKIP] Replay for task ${originalResult.case_id}: Invalid original attempt. Copying original result.`,
				)
				newResultInput.succeeded = originalResult.succeeded
				newResultInput.error_enum = originalResult.error_enum
			}
	...
	async runAllTestsForModel(
		...
		for (const testCase of testCases) {
			...
			while (validAttempts < testConfig.number_of_runs) {
				...
				log(
					isVerbose,
					`  Attempt ${totalAttempts} for ${testCase.test_id} (${validAttempts}/${testConfig.number_of_runs} valid so far)...`,
				)
				...
				if (isValid) {
					...
					log(
						isVerbose,
						`  ✓ Valid attempt ${validAttempts}/${testConfig.number_of_runs} completed (${result.success ? "SUCCESS" : "FAILED"})`,
					)
				} else {
				...
				if (totalAttempts >= testConfig.max_attempts_per_case) {
					log(
						isVerbose,
						`  ⚠️ Reached maximum attempts (${totalAttempts}) for test case ${testCase.test_id}. Only got ${validAttempts}/${testConfig.number_of_runs} valid attempts.`,
					)
					break
				}
			}
			...
			log(
				isVerbose,
				`  ✓ Completed test case ${testCase.test_id}: ${validAttempts}/${testConfig.number_of_runs} valid attempts (${totalAttempts} total attempts)`,
			)
		}
		return results
	}
...
async function main() {
	...
	const maxAttemptsPerCase = options.maxAttemptsPerCase ? parseInt(options.maxAttemptsPerCase, 10) : validAttemptsPerCase * 10
	...
	try {
		...
		if (Object.keys(openRouterModelDataGlobal).length === 0 && isVerbose) {
			log(
				isVerbose,
				"Warning: Could not load OpenRouter model data. Context window filtering might be affected for OpenRouter models.",
			)
		}

		const runner = new NodeTestRunner(options.replay, options.provider)
		const allLoadedTestCases = runner.loadTestCases(testPath, isVerbose) // Pass isVerbose
		...
		log(
			isVerbose,
			`-Target: ${validAttemptsPerCase} valid attempts per test case per model (will retry until this many valid attempts are collected)`,
		)
		if (options.replay) {
		...
		for (const modelId of modelIds) {
			...
			if (currentModelContext && currentModelContext > 0) {
				...
				log(
					isVerbose,
					`Warning: Context window for model ${modelId} is unknown or zero. It will not constrain the test case selection.`,
				)
			}
		}
		...
		if (smallestContextWindow === Infinity) {
			log(
				isVerbose,
				"Warning: Could not determine a common smallest context window. Proceeding with all loaded cases, context issues may occur.",
			)
		} else {
			log(
				isVerbose,
				`Smallest common context window (with padding consideration) across specified models: ${smallestContextWindow} (target for filtering: ${smallestContextWindow - 20000})`,
			)
		}

		let eligibleCasesForThisRun = [...allLoadedTestCases]
		if (smallestContextWindow !== Infinity && smallestContextWindow > 20000) {
			// Only filter if a valid smallest window is found
			...
			log(
				isVerbose,
				`Filtered to ${eligibleCasesForThisRun.length} cases (from ${originalCaseCount}) to fit smallest context window of ${smallestContextWindow} (with padding).`,
			)
		}
		...
		await runner.initializeMultiModelRun(
			processedEligibleCasesForRun,
		...
		while (remainingTasks.length > 0) {
			...
			const batchPromises = batch.map((task) => {
				...
				log(
					isVerbose,
					`  Attempt ${taskStates[taskId].total} for ${task.testCase.test_id} with ${task.modelId} (${taskStates[taskId].valid} valid, ${taskStates[taskId].pending - 1} pending)...`,
				)
			...
			for (const result of batchResults) {
				...
				if (runner.isValidAttempt(result)) {
					...
					log(
						isVerbose,
						`  ✓ Valid attempt ${taskStates[taskId].valid}/${validAttemptsPerCase} for ${result.test_id} with ${result.modelId} completed (${result.success ? "SUCCESS" : "FAILED"})`,
					)
				} else {
					log(
						isVerbose,
						`  ✗ Invalid attempt for ${result.test_id} with ${result.modelId} (error: ${result.error || "unknown"})`,
					)
				}
			...
			log(
				isVerbose,
				`-Completed batch... (Batch Cost: $${batchCost.toFixed(6)}, Remaining tasks: ${remainingTasks.length})`,
			)
		}
