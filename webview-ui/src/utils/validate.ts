		...
		switch (apiConfiguration.apiProvider) {
			...
			case "bedrock":
				if (!apiConfiguration.awsRegion) {
					return "You must select an authentication method and choose a region to use with AWS Bedrock"
				}
				break
			case "openrouter":
			...
			case "fireworks":
				if (!apiConfiguration.fireworksApiKey || !apiConfiguration.fireworksModelId) {
					return "You must provide a valid API key and a Model ID, or choose a different provider"
				}
				break
			case "together":
			...
			case "vscode-lm":
				if (!apiConfiguration.vsCodeLmModelSelector) {
					return "Set up Copilot and select a model"
				}
				break
			case "moonshot":
